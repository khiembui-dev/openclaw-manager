 ...
 * Proxies to Manager (port 3847) which is the web UI.
 * Gateway (WebSocket) runs on gatewayPort but is not HTTP-compatible for direct proxy.
 */
async function setupNginx(domain, gatewayPort, email, logFn) {
  const log = logFn || (() => {});
  const managerPort = require('../config').port; // Manager web UI port (3847)
  ...
  // Create nginx config - proxy to Manager web UI (3847), not Gateway WebSocket (18789)
  const nginxConf = `# OpenClaw Manager - ${domain}
# Generated by OpenClaw Manager
# Manager UI: port ${managerPort} | Gateway WS: port ${gatewayPort}
...
server {
    ...
    # Manager Web UI
    location / {
        proxy_pass http://127.0.0.1:${managerPort};
    ...
    # OpenClaw Gateway WebSocket (direct access)
    location /ws/ {
